...
int index_end(const int q, const int i_s){ return (int)(i_s + q - 1);}
...
void baryctrwt_diffmat22( const int q, const int i, const double *x, double *D2, int r );
...
/*********************************************************************/
{
    ...
    const size_t Nx = 3320;/*h5_dataset_npoints( grid_fid, "x" );*/
    const size_t Ny = 224;/*h5_dataset_npoints( grid_fid, "y" );*/
    const size_t Nz = 2048;/*h5_dataset_npoints( grid_fid, "z" );*/
    ...
    const double dx = 0.292210466252391; /*x[1] - x[0];*/
    const double dz = 0.117187500000000; /*z[1] - z[0];*/
    ...
    FILE *fr;
    char line[80];
    fr = fopen ("/Users/zzwz02/Documents/BL-baryctrwt/BL-baryctrwt/y.txt", "rt");
    i=0;
        ...
        for( i=0; i<Ny; i++ ) {
            ...
            for(r=1; r<=2; r++ ) {
                ...
                if( r==1 ) {
                    ...
                    if (i==0 && q_prime==4){
                        baryctrwt_diffmat22( q, i_local, y+i_s, D, r );
                    } else{
