...
--into turbinfo_test..usage_summary
from turbinfo_test..usage
left join turbinfo_test..opID on opID.op=usage.op
left join turbinfo_test..DataID on DataID.DatasetID= usage.dataset
left join turbinfo_test..users on users.uid= usage.uid
left join turbinfo_test..[GeoLite2-City-Blocks-IPv4] as ips on ips.ip_start<CONVERT(VARBINARY(8), usage.ip) and CONVERT(VARBINARY(8), usage.ip)<ips.ip_end
left join turbinfo_test..[GeoLite2-City-Locations-en] as ipl on ipl.geoname_id=ips.geoname_id
...
select max(rowid) as rowid, CONVERT(DATE,CAST(year(date) AS VARCHAR(4))+'-'+ CAST(month(date) AS VARCHAR(2))+'-'+CAST(day(date) AS VARCHAR(2))) as dates,
		...
		users.authkey, usage.uid,
		ipl.city_name, ipl.country_name, usage.ip
--into turbinfo_test_test..usage_summary
from turbinfo_test..usage
left join turbinfo_test..opID on opID.op=usage.op
left join turbinfo_test..DataID on DataID.DatasetID= usage.dataset
left join turbinfo_test..users on users.uid= usage.uid
...
SET @startdates = '2018-01-01'; 
DECLARE	@enddates DATE;
SET @enddates = '2018-04-01'; 
