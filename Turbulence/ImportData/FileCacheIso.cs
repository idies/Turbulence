...
namespace ImportData
{
    ...
    /// </summary>
    public class FileCacheIso : FileCache
    {
        private byte[] largeArray;
        private int cubeSize;
        private string field;

        public FileCacheIso(string data_dir, int[] resolution, long headerSize, int components, string field)
            ...
            while (exception_flag && attempts < max_attempts)
            {
                try
                {
                    ...
                    while (totalRead < dataSize)
                    {
                        ...
                        bytesToRead = 128 * cubeSize * components * pointDataSize;
                        remainingBytes = (int)(fileSize - offset);
                ...
                catch (Exception ex)
                {
                    ...
                    Console.WriteLine("Array size: {0}, offset: {1}, length: {2}", fileSize, offset, bytesToRead);
        ...
        private string GetFileName(int time, long zindex, string field)
        {
            ...
            //{
                return String.Format(@"C:\globus\{2}_vector{0:00000}.{1:000}", time,zindex, field);
            //}
            //else
            //{
