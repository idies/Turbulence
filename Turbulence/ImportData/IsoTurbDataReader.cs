...
namespace IsoImportData
{
 ...
 /// various meta data...
        ...
        int components;  // number of components for each point (4 for Ux,Uy,Uz,P)
        ...
        public struct ExportDataFormat
        {
            public ExportDataFormat(long start, long end, long inc, int length, int edge, bool isDataLittleEndian)
            {
                ...
                {
                    this.start += inc;
                    Console.WriteLine("Yo it adjusted!");
                }  
                this.end = end;
                this.inc = inc;
                this.length = length;
        ...
        public static IDataReader GetReader(int timestep, int timeoff,
            int[] resolution, int components, ExportDataFormat dataFormat, IsoFileCache cache, string data_dir)
        {
            return new IsoTurbDataReader(timestep, timeoff, resolution, components, dataFormat, cache, data_dir);
        }
         ...
         */
        internal IsoTurbDataReader(int timestep, int timeoff,
            int[] resolution, int components, ExportDataFormat dataFormat, IsoFileCache cache, string data_dir)
        {
            this.done = false;
            ...
            //cache = new IsoFileCache(data_dir); //, resolution
            //this.cache = cache;
        }
        ...
        private void ReadData()
        {
            ...
            else
            {
                ...
                //Console.WriteLine("Bz, by, and bx are: {0}, {1}, {2}", bz, by, bx);
                //Console.WriteLine("dataFormat.edge = {0}, corner[2] = {1}, datacounter={2} Press enter", dataFormat.edge, corner[2], datacounter);
                //if (bx == 0 && by == 0)
                //{
                //    Console.WriteLine("BX and BY is 0, hit enter");
                //    Console.ReadLine();
               // }
                //Console.ReadLine();
                // upper (end) corner
                ...
                //Console.WriteLine("data length = {0}", dataFormat.length);
                ...
                //Console.WriteLine("Source data size is {0}", data.Length);
                cache.readFiles(timestep, bz/8, 1);
                //Throw error if file is missing!!!
                //Verify file exists.
                
                if (cache.filenotfound)
                {      
                    //This helps to prevent writes when the file is missing so we don't produce ghost entries.
                    throw new IOException(String.Format("File missing"));
                }
                ...
                for (int z = bz; z != ez; z = (z + 1) % resolution[0])
                {
                    ...
                    for (int y = by; y != ey; y = (y + 1) % resolution[1])
                    {
                        ...
                        for (int x = bx; x != ex; x = (x + 1) % resolution[2])
                        {
                            ...
                            //Start at one because the first byte is junk.
                            //0 is junk, ux,uy,uz is 1,2,3, and 4 is pressure. 
                            // This was for velocity: for (int i = 1; i < components+1; i++)
                            int start = 4; //This is for pressure.  If it is velocity (3 components) it gets changed below.
                            int end = 5;
                            if (this.components == 3)
                            {
                                start = 1; //this is for velocity
                                end = 4;
                            }
                            ...
                            for (int i = start; i < end; i++)
                            {
                                ...
                                //if (bufferOffset < 200)
                                //{
                                   //Console.WriteLine("value = {0} i={1}", System.BitConverter.ToSingle(data, bufferOffset), i);
                                //}
