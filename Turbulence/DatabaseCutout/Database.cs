...
namespace DatabaseCutout
{
    ...
    public class Database
    {
        ...
        public void selectServers(DataInfo.DataSets dataset_enum)
        {
            ...
            String cString = "Server=gw01;Database=turbinfo;Asynchronous Processing=false;User ID=turbquery;Password=***REMOVED***;Pooling=true;Max Pool Size=250;Min Pool Size=20;Connection Lifetime=7200";
        ...
        /// <param name="points">Points</param>
        /// <param name="round">True for round, false for floor</param>
        public void AddBulkTrackingParticles(TrackingInfo[] points, bool round, int nOrder)
        {
            for (int i = 0; i < points.Length; i++)
            {
                if (!points[i].done)
                {
                    if (points[i].compute_predictor)
                        AddWorkloadTrackingPointToMultipleServers(i, points[i].position.z, points[i].position.y, points[i].position.x, points[i], round, nOrder);
                    else
                        AddWorkloadTrackingPointToMultipleServers(i, points[i].predictor.z, points[i].predictor.y, points[i].predictor.x, points[i], round, nOrder);
                }
            }
            DoBulkInsert();
        }
        ...
        private void GetRawResults(byte[] result, int components,
            ...
            int[] serverX, int[] serverY, int[] serverZ, int[] serverXwidth, int[] serverYwidth, int[] serverZwidth)
        {
            ...
            for (int s = 0; s < serverCount; s++)
            {
                if (connections[s] != null)
                {
                    ...
                    SqlDataReader reader = sqlcmds[s].ExecuteReader();
                    while (reader.Read())
                    {
                        int bytesread = 0;
        ...
        private void ExecuteGetRawData(string dataset,
            int timestep,
            int[] serverX, int[] serverY, int[] serverZ, int[] serverXwidth, int[] serverYwidth, int[] serverZwidth)
        {
            // initiate reader requests
        ...
        public int ExecuteGetPosition(string dataset, float time,
            TurbulenceOptions.SpatialInterpolation spatial,
            TurbulenceOptions.TemporalInterpolation temporal,
            ...
            int X, int Y, int Z, int Xwidth, int Ywidth, int Zwidth)
        {
            ...
            GetRawResults(result, components, X, Y, Z, Xwidth, Ywidth, Zwidth, serverX, serverY, serverZ, serverXwidth, serverYwidth, serverZwidth);
