...
namespace ImportData
{
    ...
    public class ChannelDataReader : IDataReader
    {
        ...
        private static string[] colNames =
            { "timestep", "zindex", "data" };
        private static Type[] colTypes =
            { typeof(string), typeof(long), typeof(byte[]) };
        private static string[] colTypeNames =
            { "int", "long", "binary(6168)" };
        byte[] data;
        ...
        {
            public ExportDataFormat(long start, long end, long inc, int length, bool isDataLittleEndian)
            {
                ...
                if (this.start < start)
                    this.start += inc;
                // Similarly, we could adjust end, but it shouldn't matter.
                this.end = end;
                this.inc = inc;
                this.length = length;
        ...
        public static IDataReader GetReader(int timestep, int timeoff,
            int[] resolution, int components, ExportDataFormat dataFormat, HDF5Reader hdf5reader)
        {
            return new ChannelDataReader(timestep, timeoff, resolution, components, dataFormat, hdf5reader);
        }
         ...
         */
        internal ChannelDataReader(int timestep, int timeoff,
            int[] resolution, int components, ExportDataFormat dataFormat, HDF5Reader hdf5reader)
        {
            this.done = false;
            ...
            data = new byte[SqlArrayHeaderSize +  dataFormat.length * dataFormat.length * dataFormat.length * components * pointDataSize];
        ...
        /// <summary>
        /// Generates the SqlArray header. The header bits are stored in the header[] array.
        /// Also sets the SqlArrayHeaderSize.
        /// </summary>
        private void GenerateSqlArrayHeader()
        {
            int cube_size = dataFormat.length * dataFormat.length * dataFormat.length;
	        if (cube_size > 8000)
            {
                int cube_width = dataFormat.length;
        ...
        private void ReadDataFromCache()
        {
            ...
            for (int z = bz; z != ez; z = (z + 1) % resolution[0])
            {
                ...
                long zadj = (z + resolution[0] - cache.Base[0]) % resolution[0];
                for (int y = by; y != ey; y = (y + 1) % resolution[1])
                {
                    // adjusted y offset based on base of file cache
                    long yadj = (y + resolution[1] - cache.Base[1]) % resolution[1];
                    for (int x = bx; x != ex; x = (x + 1) % resolution[2])
                    {
                        // adjusted x offset based on base of file cache
                        long xadj = (x + resolution[2] - cache.Base[2]) % resolution[2];
                        long fileoff = (zadj * resolution[2] * resolution[1] + yadj * resolution[2] + xadj) * pointDataSize;
                        for (int i = 0; i < components; i++)
                        {
                            ...
                            //Console.WriteLine("value = {0}", System.BitConverter.ToDouble(data, bufferOffset));
        ...
        private void ReadHDF5Data() {
            int[] corner = new Morton3D(datacounter).GetValues();
            
            // The beginning of the data atom contains the SqlArray header.
            hdf5reader.GetAtom(new int[] { corner[0] - hdf5reader.Base[0], corner[1] - hdf5reader.Base[1], corner[2] - hdf5reader.Base[2] },
                new int[] { dataFormat.length, dataFormat.length, dataFormat.length }, data, SqlArrayHeaderSize);
        }
        ...
        public bool Read()
        {
            ...
            if (!done)
            {
                ...
                if (step < steps)
                {
                    ...
                    // TODO: Should be parameter based. If we are dealing with HDF5 data then use this method,
                    // otherwise read the data from the file cache.
                    ReadHDF5Data();
                }
                else
                {
