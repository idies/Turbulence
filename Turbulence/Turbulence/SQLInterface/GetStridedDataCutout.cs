...
public partial class StoredProcedures
{
        ...
        string QueryBox)
    {
        ...
        try
        {
            ...
            GetCutout(table, dbname, timestep, coordinates, contextConn, stride, out cutout);
    ...
    private static void GetCutout(TurbDataTable table, string dbname, int timestep, int[] coordinates, SqlConnection connection, int stride, out byte[] cutout)
    {
        int x_width, y_width, z_width, x, y, z;
        x_width = (coordinates[3] - coordinates[0] - 1) / stride + 1;
        y_width = (coordinates[4] - coordinates[1] - 1) / stride + 1;
        z_width = (coordinates[5] - coordinates[2] - 1) / stride + 1;
        ...
        using (SqlDataReader reader = command.ExecuteReader())
        {
            while (reader.Read())
            {
                ...
                GetSourceDestLen(x, coordinates[0], coordinates[3], atomWidth, stride, ref sourceX, ref destinationX, ref lengthX);
                GetSourceDestLen(y, coordinates[1], coordinates[4], atomWidth, stride, ref sourceY, ref destinationY, ref lengthY);
                GetSourceDestLen(z, coordinates[2], coordinates[5], atomWidth, stride, ref sourceZ, ref destinationZ, ref lengthZ);
                
                for (int k = 0; k < lengthZ; k += stride)
                {
                    for (int j = 0; j < lengthY; j += stride)
                    {
                        int source = ((sourceZ + k) * atomWidth * atomWidth + (sourceY + j) * atomWidth + sourceX) * table.Components * sizeof(float);
                        int dest = ((destinationZ + k / stride) * x_width * y_width + (destinationY + j / stride) * x_width + destinationX) * table.Components * sizeof(float);
                        for (int i = 0; i < lengthX; i += stride)
                        {
                            Array.Copy(rawdata, source, cutout, dest, table.Components * sizeof(float));
                            source += stride * table.Components * sizeof(float);
