...
//{
    ...
    /// </summary>
    public class CutoutDataReader : IDataReader
    {
        /// various meta data...
        ...
        /* Create a buffer instead of a cache */
        //FileCache cache;
        byte[] cutout;
        ...
        protected int[] cutout_dimensions;
        ...
        {
            public ExportDataFormat(long start, long end, long inc, int length)
            {
        ...
        public static IDataReader GetReader(int timestep, int timeoff,
            int[] resolution, int components, ExportDataFormat dataFormat, byte[] cutout)
        {
            return new CutoutDataReader(timestep, timeoff, resolution, components, dataFormat, cutout);
        }
         ...
         */
        internal CutoutDataReader(int timestep, int timeoff,
            int[] resolution, int components, ExportDataFormat dataFormat, byte[] cutout)
        {
            this.done = false;
            ...
            this.cutout_dimensions = this.resolution;
            // counter = -1;
            step = -1;
        ...
        private void GenerateSqlArrayHeader()
        {
            ...
            if (cube_size > 8000)
            {
                int cube_width = dataFormat.length + 2;
        ...
        private void ReadData()
        {
            ...
            // lower (base) corner
            int bz = (corner[0] + resolution[0]) % resolution[0];
            int by = (corner[1] + resolution[1]) % resolution[1];
            int bx = (corner[2] + resolution[2]) % resolution[2];
            // upper (end) corner
            int ez = (corner[0] + dataFormat.length ) % resolution[0];
            int ey = (corner[1] + dataFormat.length ) % resolution[1];
            int ex = (corner[2] + dataFormat.length ) % resolution[2];
