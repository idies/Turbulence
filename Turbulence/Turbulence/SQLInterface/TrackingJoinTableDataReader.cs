...
{
    public class TrackingJoinTableDataReader : IDataReader
    {
        /// various meta data...
        private static string[] colNames = { "basetime", "zindex" };
        private static Type[] colTypes = { typeof(int), typeof(long) };
        private static string[] colTypeNames = { "int", "long" };
        private bool done;  // set when done reading data

        List<SQLUtility.TimestepZindexKey> indexes; // indexes to be written to the database
        ...
        * marked as internal.
        */
        internal TrackingJoinTableDataReader(ICollection<SQLUtility.TimestepZindexKey> keys, float points_per_cube,
            Turbulence.TurbLib.TurbulenceOptions.TemporalInterpolation temporalInterpolation, int timeInc)
        {
            this.done = false;
            this.indexes = new List<SQLUtility.TimestepZindexKey>(keys);
            this.recordsToGenerate = indexes.Count;
            this.timestepsForInterpolation = 1;
            this.timeInc = timeInc;

            if (temporalInterpolation == TurbLib.TurbulenceOptions.TemporalInterpolation.PCHIP)
            {
                colNames = new string[] { "basetime", "zindex", "timestep" };
                colTypes = new Type[] { typeof(int), typeof(long), typeof(int) };
                colTypeNames = new string[] { "int", "long", "int" };
                timestepsForInterpolation = 4;
                recordsToGenerate = indexes.Count * timestepsForInterpolation;
            }
        ...
        public static IDataReader GetReader(ICollection<SQLUtility.TimestepZindexKey> keys, float points_per_cube, 
            Turbulence.TurbLib.TurbulenceOptions.TemporalInterpolation temporalInterpolation, int timeInc)
        {
            return new TrackingJoinTableDataReader(keys, points_per_cube, temporalInterpolation, timeInc);
        }
        ...
        public bool Read()
        {
            ...
            if (!done)
            {
                if (recordsProcessed == recordsToGenerate)
                    done = true;
                else
                {
                    zindex = indexes[recordsProcessed / timestepsForInterpolation].Zindex;
                    this.basetime = indexes[recordsProcessed / timestepsForInterpolation].Timestep;
                    this.timestep = basetime - timeInc + (recordsProcessed % timestepsForInterpolation) * timeInc;
