    ...
    public class GetQ : GetMHDGradient
    {
            ...
            int[] cutout_coordinates, int[] cutout_dimensions, int offset_y)
        {
            ...
            for (int x_i = startx; x_i <= endx; x_i++)
            {
                ...
                gradient[0] += coeff * GetDataItem(sourceIndex);     // computes dux/dx
                gradient[3] += coeff * GetDataItem(sourceIndex + 1); // computes duy/dx
                gradient[6] += coeff * GetDataItem(sourceIndex + 2); // computes duz/dx
            ...
            for (int y_i = starty; y_i <= endy; y_i++)
            {
                ...
                gradient[1] += coeff * GetDataItem(sourceIndex);     // computes dux/dy
                gradient[4] += coeff * GetDataItem(sourceIndex + 1); // computes duy/dy
                gradient[7] += coeff * GetDataItem(sourceIndex + 2); // computes duz/dy
                point.numPointsProcessed++;
            }
            for (int z_i = startz; z_i <= endz; z_i++)
            {
                ...
                ulong sourceIndex = (((ulong)z_i - (ulong)cutout_coordinates[2]) * (ulong)cutout_dimensions[2] * (ulong)cutout_dimensions[1] +
                    ((ulong)y - (ulong)cutout_coordinates[1]) * (ulong)cutout_dimensions[2] +
                    ((ulong)x - (ulong)cutout_coordinates[0])) * (ulong)setInfo.Components;
                gradient[2] += coeff * GetDataItem(sourceIndex);     // computes dux/dz
                gradient[5] += coeff * GetDataItem(sourceIndex + 1); // computes duy/dz
                gradient[8] += coeff * GetDataItem(sourceIndex + 2); // computes duz/dz
                point.numPointsProcessed++;
            }
