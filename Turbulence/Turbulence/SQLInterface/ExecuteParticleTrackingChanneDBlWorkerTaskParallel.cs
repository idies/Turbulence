...
public partial class StoredProcedures
{
    ...
    public static void ExecuteParticleTrackingChannelDBWorkerTaskParallel(
        string codedb,
        string turbinfodb,
            ...
            while (!all_done)
            {
                ...
                for (int s = 0; s < servers.Count; s++)
                {
                    if (map[s].Count > 0)
                    {
                        ...
                        {
                            string pathSource = SQLUtility.getDBfilePath(databases[s], timesteps[i], table.DataName, connections[s]);
